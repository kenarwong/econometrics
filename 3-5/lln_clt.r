...
rPath <- "/mnt/store/learn/econometrics/code/3-5/"

# Plot of Bernoulli Random Variable Mean as sample size reaches 1000
n <- 1000
p <- 0.5
bernoulli_samples <- rbinom(n, size = 1, prob = p)
sample_means <- cumsum(bernoulli_samples) / seq_along(bernoulli_samples) # Running mean
...
ggsave(file.path(rPath, "LLN_bernoulli.png"))

# Plot of Bernoulli Random Variable as sample size reaches 1000
clt_bernoulli <- function() {
  par(mfrow=c(2,2))
  # set.seed(123)
  
  sample_sizes <- c(10, 100, 500, 1000)
  
  # for each sample size, generate 1000 samples and plot the histogram of proportions of successes
...
# Save as LLN_clt.png
dev.copy(png, file.path(rPath, "CLT_bernoulli.png"))
dev.off()
...
ggsave(file.path(rPath, "LLN_cauchy.png"))
...
dev.copy(png, file.path(rPath, "CLT_cauchy.png"))
dev.off()